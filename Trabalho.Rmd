...
gbd_filtered <- gbd_filtered %>%
        ...
        ungroup()
# Fazer Barplot
ggplot(data = gbd_filtered, aes(x = location, y = val,color = measure, group = measure)) +
        geom_line(size = 1) +
        geom_point(size = 3) +
        geom_text(aes(x = location, y = val, label = rank), vjust = -1.5, size = 3, fontface = "bold", inherit.aes = FALSE) +
        labs(
                title = "Taxas Age-standardized para Doenças Respiratórias Crônicas (2021)",
                x = "Região (ordenada por valor decrescente de death)",
                y = "Valor (Taxa)",
                color = "Medida",
                caption = "Data source: GBD 2021"
        ) +
...
gbd_filtered <- gbd_total %>% filter(
        ...
        year == "2021")
# Fazer Barplot
ggplot(data = gbd_filtered, aes(x = factor(location,
                levels = c("Low SDI", "Low-middle SDI", "Middle SDI","High-middle SDI", "High SDI")), 
                y = val)) +
        geom_bar(stat = "identity", width = 0.6, fill = "steelblue") +
        geom_errorbar( aes(x = factor(location,
                levels = c("Low SDI", "Low-middle SDI", "Middle SDI", "High-middle SDI", "High SDI")),
...
gbd_filtered <- gbd_total %>% filter(
        ...
        metric == "Rate")
# Fazer Barplot
ggplot(data = gbd_filtered, aes(x = year, 
                y = val,color=factor(location,
                levels = c("Low SDI", "Low-middle SDI", "Middle SDI","High-middle SDI", "High SDI")))) +
        ...
        #geom_errorbar( aes(x = year,
                #ymin=lower, ymax=upper), 
                #width=0.4, colour="orange", alpha=0.9, size=0.5) +
        labs(title = "Taxa de Mortes por ano por SDI",
                x = "Local", y = "Taxa", color="Nível de SDI") +
        theme_minimal()
```
...
ggplot(data = gbd_filtered, aes(x = age, y = val)) +
        ...
        geom_errorbar( aes(x=age, ymin=lower, ymax=upper), 
                width=0.4, colour="orange", alpha=0.9, size=0.5) + 
        labs(title = "Taxa de Mortes por idade",
                x = "Idade", y = "Taxa de mortes") +
        theme_minimal()
```
...
ggplot(data = gbd_filtered, aes(x = age, y = val, fill = sex)) +  # Adicionado fill = sex para agrupamento
        ...
        labs(title = "Taxa de Mortes por Idade e Sexo (Poluição do Ar, Global, 2021)",
                x = "Idade", y = "Taxa de mortes", fill = "Sexo") +
        theme_minimal() +
...
gbd_filtered<-mutate(gbd_filtered,location = fct_reorder(location, val))
# Fazer Barplot
ggplot(data = gbd_filtered, aes(x = age, y = val, fill = location)) +  # Adicionado fill = sex para agrupamento
        geom_bar(stat = "identity", width = 0.6, position = position_dodge(width = 0.6)) +  # position_dodge para barras lado a lado
        geom_errorbar(aes(ymin = lower, ymax = upper), 
                position = position_dodge(width = 0.6),  # Alinhar barras de erro com as barras dodged
                width = 0.4, colour = "orange", alpha = 0.9, size = 0.5) + 
        labs(title = "Taxa de Mortes por Idade e Sexo (Poluição do Ar, Global, 2021)",
                x = "Idade", y = "Taxa de mortes", fill = "SDI") +
        theme_minimal() +
        theme(legend.position = "right")
```
...
# Fazer Barplot
ggplot(data = gbd_filtered, aes(x = year, y = val, fill = rei)) +
        geom_bar(stat = "identity", width = 0.7, position = position_dodge(width = 0.8)) +  # position_dodge para barras lado a lado
        geom_errorbar(aes(ymin = lower, ymax = upper), 
                position = position_dodge(width = 0.8),  # Alinhar barras de erro com as barras dodged
                width = 0.4, colour = "orange", alpha = 0.9, size = 0.5) +
        facet_grid(sex ~ ., scales = "fixed") +
        labs(title = "Taxa de Mortes por Idade e Sexo (Poluição do Ar, Global, 2021)",
                x = "Idade", y = "Taxa de mortes", fill = "Fator de risco") +
        theme_minimal() +
        theme(legend.position = "right")
```
